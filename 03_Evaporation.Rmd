...
\begin{equation}
E_A = f_e (\bar{u_r})(e^{\star}_s - e^{\star}_a)  
\end{equation}  
...
where $u_2$ is the wind speed at $h=2m$.  

Further investigation has shown potential evaporation to be a simplification of 

### 8.3 Catchment Method  
Based on the water budget method, the catchment method is applicable over periods where you can assume storage to be zero; this is typically a year.  Thus, for a watershed, the evaporation can be estimated by:  

\begin{equation}
P-G_{out}+G_{in}+Q_{in}-Q_{out}-ET=\Delta S \approx 0
\end{equation}

Given that for a watershed, $Q_{in} = 0$, and for no storage and over this period, $G_{in} = G_{out}$.  
